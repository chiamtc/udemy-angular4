...
export class DataStorageService {
  ...
  storeRecipes() {
    ...
    //   // headers: headers
    // });
    const req = new HttpRequest('PUT', 'https://ng-rx-3c367.firebaseio.com//recipes.json', this.recipeService.getRecipes(), {reportProgress: true});
    return this.httpClient.request(req);
  }

  getRecipes() {
    // this.httpClient.get<Recipe[]>('https://ng-recipe-book-3adbb.firebaseio.com/recipes.json?auth=' + token)
    this.httpClient.get<Recipe[]>('https://ng-rx-3c367.firebaseio.com//recipes.json', {
      observe: 'body',
      responseType: 'json'
    })
      .map(
